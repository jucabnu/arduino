...
void readBT(){
  ...
  int i = 0;
  while (Serial.available()) // if date from softwareserial Bt module
  {
    if(Serial.available() > 0) // reading data into char array 
    {
      count++;
      char c = Serial.read();
      if (c== '1'){
...
void writeBT(){
  ...
  if (Serial.available())  // if data on hardwareserial port PC or notebook
    ...
    Serial.write(Serial.read());  // write it to the BT module
}
*/
